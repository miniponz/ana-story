...
function printPartOne(){
    ...
    showStoryPartOne.textContent = storyPartOne;
    //need to figure out how to control where text is written
}
function printPartTwo(){
    var storyPartTwo = 'Ana was preparing to master the world. Her plan was to make a lick launcher that stretched and launched her tongue so she could use it to lick all of the life out of the humans. Her plan to make the lick launcher was to build a tongue extender-jig-a-ma-bob, then add a lick intensifier into the cortex of the tongue extender (we don\'t know what any of that meant).'

    var showStoryPartTwo = document.getElementById('part-two');

    showStoryPartTwo.textContent = storyPartTwo;
}
function printPartThree(){
    var storyPartThree = 'After the lick cannon was built, Ana decided to use it on Riley first. Alas, it worked! Riley was laying doge cold on the floor, but Riley was alive. Riley withstood the lick. Ana decided to use it on all of the world! '

    var showStoryPartThree = document.getElementById('part-three');

    showStoryPartThree.textContent = storyPartThree;
}
